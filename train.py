...
for n_fold in range(0,args.n_fold):
    ...
    for model_index in range(0,args.ensemble):
        ...
        for epoch in range(0, args.n_epochs):
            ...
            if val_loss <= best_val_loss:
                ...
                best_epoch = epoch
                torch.save(model.state_dict(), os.path.join(args.log_dir,f"Fold_{n_fold}" ,f"Model_{model_index}",'best_model'))
    ...
    # predict test data
        test_loader = construct_loader(args, n_fold, modes='test')
    ...
    # save predictions
        ...
        preds_path = os.path.join(args.log_dir, f"Fold_{n_fold}" ,f"Model_{model_index}", 'preds.csv')
        pd.DataFrame(list(zip(smiles, preds)), columns=['smiles', 'prediction']).to_csv(preds_path, index=False)

preds_out = numpy.array(preds_out)
preds_out = numpy.mean(preds_out,axis=0)
preds_out = preds_out.tolist()
preds_path = os.path.join(args.log_dir, 'preds.csv')
pd.DataFrame(list(zip(smiles, preds_out)), columns=['smiles', 'prediction']).to_csv(preds_path, index=False)